...
public abstract class ProdutoDao{
    ...
    public static Produto buscarProduto(int id) {
        try {
            em.getTransaction().begin();
            //Busca um registro por ID, o atributo Produto.class serve pra identificar que é uma classe do tipo Produto
    ...
    public static List<Produto> buscarProdutoPorNome(String nome) {
        try {
            ...
            Query sql = em.createQuery("SELECT p FROM Produto p WHERE p.nome LIKE :nome");
